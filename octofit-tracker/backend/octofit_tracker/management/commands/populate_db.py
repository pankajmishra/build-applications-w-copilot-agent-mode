...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        # Delete all existing data in correct order to avoid FK issues and unhashable errors
        try:
            Activity.objects.all().delete()
        except Exception as e:
            self.stdout.write(self.style.WARNING(f'Could not delete Activities: {e}'))
        try:
            Leaderboard.objects.all().delete()
        except Exception as e:
            self.stdout.write(self.style.WARNING(f'Could not delete Leaderboards: {e}'))
        try:
            Workout.objects.all().delete()
        except Exception as e:
            self.stdout.write(self.style.WARNING(f'Could not delete Workouts: {e}'))
        try:
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f'Could not delete Teams: {e}'))

        # Re-fetch objects after deletion to avoid unhashable errors
        # Create Teams
        ...
        # Ensure unique index on email field in users collection (collection name should be 'user' or 'users')
